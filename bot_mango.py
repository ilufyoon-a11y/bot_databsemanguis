...
# --- 2. CONEXIÓN A GOOGLE SHEETS (MÉTODO REFORZADO) ---
sheet = None
...
def conectar_google():
    ...
    try:
        ...
        if not creds_json:
            print("🚨 ERROR: No se encontró la variable GOOGLE_CREDS")
            return

        # Cargamos el JSON
        info = json.loads(creds_json)
        
        # ARREGLO CRÍTICO: Google necesita que los \n sean saltos de línea reales
        info['private_key'] = info['private_key'].replace('\\n', '\n')
        
        creds = ServiceAccountCredentials.from_json_keyfile_dict(info, scope)
        client = gspread.authorize(creds)
        
        # Conexión a mango / datos
...
# --- 3. LÓGICA DEL BOT (Tus 9 pasos intactos) ---
...
async def finalizar(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if sheet is None:
        await update.message.reply_text("❌ Error: Sigo sin conexión. Revisa Render.")
    ...
    try:
        ...
        await update.message.reply_text("✅ **¡GUARDADO EXITOSO!** 🥭💜", parse_mode='Markdown')
    except Exception as e:
        await update.message.reply_text(f"❌ Error al guardar: {e}")
    return ConversationHandler.END

async def cancel(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text("❌ Cancelado. Borahae! 💜", reply_markup=ReplyKeyboardRemove())
    return ConversationHandler.END
