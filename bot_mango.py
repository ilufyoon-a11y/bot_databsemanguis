...
def conectar_google():
    ...
    try:
        # Leemos el JSON desde la variable de entorno de Render
        ...
        # Cargamos la info y limpiamos la private_key (esto evita el error de firma JWT)
        ...
        # Conexión a la hoja 'mango' y pestaña 'datos'
...
async def finalizar(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if sheet is None:
        await update.message.reply_text("❌ Error crítico: Sin conexión a Google. Revisa Render.")
    ...
    try:
        ...
        await update.message.reply_text(
            f"✅ **REGISTRO EXITOSO** 💜\n\nDatos guardados en la hoja `mango`.",
            parse_mode='Markdown',
            reply_markup=ReplyKeyboardRemove()
        )
    except Exception as e:
        await update.message.reply_text(f"❌ Error al guardar en Excel: {e}")
